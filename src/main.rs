...
use rocket::{State, get, response::status, routes, tokio};
...
fn merge_servers(upstream_servers: &Servers, my_servers: &Servers) -> (Servers, String) {
    ...
    let mut digest = sha256::digest(
        serde_json::to_string_pretty(&servers).expect("Failed to serialize merged list"),
    );
    digest.truncate(8);
    digest.push('\n');
    (servers, digest)
...
async fn main() -> Result<(), rocket::Error> {
    ...
    let servers_url: Url = args.url.as_ref().map_or(
        DEFAULT_SERVERS_URL
            .try_into()
            .expect("Failed to parse default server URL (bug)"),
        |u| u.clone(),
    );
    ...
    let my_servers =
        serde_json::from_reader(my_servers_file).expect("Failed to parse servers file");
    info!("Our server list loaded");
    let servers_upstream = reqwest::get(servers_url.clone())
        .await
        .expect("Failed to fetch initial server list")
        .json::<Servers>()
        .await
        .expect("Failed to parse initial server list");
    info!("Upstream servers loaded");
    let cache_url = args
        .base_url
        .join("servers.json.hash")
        .expect("Failed to create hash url from base url");
                    ...
                    Ok(response) => {
                        ...
                        match response_json {
                            Ok(servers) => {
                                ...
                                (server_state.merged_servers, server_state.digest) =
                                    merge_servers(&servers, &server_state.my_servers);
